...
{
    public partial class InterviewsUserControl : UserControl
    {
        ...
        private EventHandler contentFunction;
        ...
        public InterviewsUserControl(GraphicFeatures graphicFeatures, EventHandler contentFunction, ContentManager contentManager)
        {
            InitializeComponent();
            ...
            this.sort_methods = new string[2] { "Time (from newest)", "Time (from oldest)" };

            this.sortFactory = new AlphabetAndTimeSortFactory();

            sortings = new (string, string)[2] { ("time", "desc"), ("time", "asc") };
        ...
        public int FillInContent()
        {
            int padding = 14;
            Content[] content = contentManager.GetContent(search_query, sort_method, sort_order, "Interview", load_number + 1);
            showMoreButton = CommonData.CheckForShowMoreButton(ref content, load_number);

            GraphicNewsAndInterview[] graphicNews = new GraphicNewsAndInterview[content.Length];
            for (int i = 0; i < content.Length; i++)
            {
                graphicNews[i] = new GraphicNewsAndInterview(content[i], graphicFeatures, contentFunction);
            }

            WhiteLinesScroll whiteLineScroll = new WhiteLinesScroll(new Size(this.Width - padding * 2, noFilterActionMenuLayout.AddNewButton.Location.Y - (noFilterActionMenuLayout.SortByBlackPanel.Location.Y + noFilterActionMenuLayout.SortByBlackPanel.Height) - 20), new Point(padding, noFilterActionMenuLayout.SortByBlackPanel.Location.Y + noFilterActionMenuLayout.SortByBlackPanel.Height + 10), graphicNews, 20, showMoreButton, graphicFeatures, LoadMoreFunction);
            noFilterActionMenuLayout.FillContent(whiteLineScroll);
            return 1;
        }
