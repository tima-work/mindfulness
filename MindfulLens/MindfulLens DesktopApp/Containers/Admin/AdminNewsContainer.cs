...
{
    public class AdminNewsContainer : UserControl, IContentContainer
    {
        protected NewsUserControl newsUserControl;
        protected ContentAdd contentAdd;
        protected ContentEdit contentEdit;
        protected User user;
        protected ContentManager contentManager;

        public AdminNewsContainer(Size size, GraphicFeatures graphicFeatures, User loggedUser, ContentManager contentManager)
        {
            this.Name = "News";
            this.contentManager = contentManager;
            //exercisesUserControl = new AdminExercisesUserControl(graphicFeatures, WatchExercise, exerciseManager);
            newsUserControl = new NewsUserControl(graphicFeatures, WatchContent, contentManager);
            ...
            newsUserControl.noFilterActionMenuLayout.AddNewButton.BindFunction(GoToAddNewContent);
            user = loggedUser;
            Size = size;
            newsUserControl.Location = new Point(0, 0);
            Controls.Add(newsUserControl);
        }
        ...
        protected void AddNewContent(object sender, EventArgs e)
        {
            try
            {
                ...
                Content content = new Content(contentAdd.DescriptionTextBox.textBox.Text, user, contentAdd.NameTextBox.textBox.Text, "News", contentAdd.image);
        ...
        {
            return newsUserControl.FillInContent();
        }
    }
}
